...
def recomendacion(titulo: str):
    ...
    df1.loc[:, list_columns] = df1[list_columns].applymap(lambda x: tuple(ast.literal_eval(x)))
    ...
    textos_concatenados =textos_concatenados = df1['overview'].apply(clean_text) + ' ' + df1['genres'].apply(lambda x: ' '.join(x)) + ' ' + \
    ...
    if len(indices) > 1:
        ...
        # Seleccionar la película ingresada
        indice = indices[0]
